...
void Actor::Update(std::vector<Platform> platforms)
{
	...
	for (int i = 0; i < platforms.size(); i++)
	{
		if (SphereOBBCol(collider, platforms[i].collider))
		{
			...
			float playerUp = Vector3D(rotation.matrixTab4[1][0], rotation.matrixTab4[1][1], rotation.matrixTab4[1][2]).Dot(Vector3D(*collider.position - *platforms[i].collider.position));
